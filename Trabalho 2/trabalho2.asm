...
msg_final db "Numero de bytes digitados:", 0
tam_msg_final EQU $-msg_final
espaco db " ", 0dh, 0ah
tam_espaco EQU $-espaco
msg_erro_arquivo db "Erro ao abrir o arquivo", 0dH, 0ah
...
simu_add:
        ...
        ;movzx esi, word[vetor_simulador+ebx]
        mov eax, vetor_simulador
        add eax, ebx
        mov esi, eax
...
igual:  ;vem pra ca
        mov ebx, [array_index]
        mov ax, "-" ;COLOCAR BARRA PARA CASOS COM NÚMEROS NEGATIVOS
        mov [array + ebx], ax
        mov ebx, [array_index]
        inc ebx
        mov [array_index], ebx
...
printa_arquivo:
        ...
        mov ecx, [esp+4] ;saindo a entrada normal por enquanto 
        mov edx, [esp+8]
        int 80h
...
string_to_int:; passar para esi e ecx, ecx= tamanho 
                ;esi = palabra 
        ;mov esi, [esp+4] ; palavra
        ;mov ecx, [esp+8] ; tamanho        
